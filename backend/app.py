...
# ---------------------------------------------------------------------------
# Strategy 数据接口（Phase 1 数据层）
# ---------------------------------------------------------------------------

@app.get("/api/strategy/market-amount")
def strategy_market_amount():
    """市场成交额数据：沪深成交额、变化率、均值。"""
    try:
        from market_amount import get_market_amount
        return {"data": get_market_amount()}
    except Exception as e:  # noqa: BLE001
        raise HTTPException(502, f"成交额数据异常：{e}") from e


@app.get("/api/strategy/sector-stats")
def strategy_sector_stats():
    """板块统计数据：成交额、占比、涨跌、涨停数等。"""
    try:
        from sector_stat import get_sector_stats
        return {"data": get_sector_stats()}
    except Exception as e:  # noqa: BLE001
        raise HTTPException(502, f"板块数据异常：{e}") from e


@app.get("/api/strategy/leader-candidates")
def strategy_leader_candidates(limit: int = 30):
    """龙头候选股票列表。"""
    try:
        from leader_candidate import get_leader_candidates
        return {"data": get_leader_candidates(limit)}
    except Exception as e:  # noqa: BLE001
        raise HTTPException(502, f"龙头候选异常：{e}") from e
...
# ---------------------------------------------------------------------------
# brain 决策简报（八层推理链 + L0 叙事 + 唯一决策结论）
# ---------------------------------------------------------------------------

@app.get("/api/brain-report")
def brain_report():
    """返回 brain 决策简报 JSON（由 run_daily.py step2b 产出）。
    包含 L0 叙事 / 决策结论(can_buy/position_pct) / 置信度 / 冲突检测 / 推理链。"""
    path = os.path.join(os.path.dirname(__file__), "output", "brain_report.json")
    if not os.path.exists(path):
        raise HTTPException(404, "决策简报尚未生成，请先运行 python run_daily.py")
...
def decision_tree():
    """返回八层决策树 JSON（由 run_daily.py step2 产出）。
    包含 L1-L5 + 情绪验证 + 板块主线 + 候选范围。"""
    path = os.path.join(os.path.dirname(__file__), "output", "decision_tree.json")
    if not os.path.exists(path):
        raise HTTPException(404, "决策树尚未生成，请先运行 python run_daily.py")
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
