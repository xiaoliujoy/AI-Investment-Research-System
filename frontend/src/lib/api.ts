...
export interface MacroAsset {
  ...
  price: number | null; change_pct: number | null; time: string;
}

export interface MarketAmountData {
  date: string;
  sh_amount: number;
  sz_amount: number;
  total_amount: number;
...
export interface BrainReport {
  ...
  L0: {
    ...
    global_narrative?: {
      ...
      q2_evidence?: string;
      q3_consensus?: string;
      q4_falsification?: string;
      q5_flow?: string;
      upcoming_catalysts?: unknown;
      falsification_catalysts?: unknown;
...
export const api = {
  ...
  strategyMarketAmount: () => get<MarketAmountData>("/strategy/market-amount"),
  strategySectorStats: () => get<SectorStat[]>("/strategy/sector-stats"),
  strategyLeaderCandidates: (limit = 30) => get<LeaderCandidate[]>(`/strategy/leader-candidates?limit=${limit}`),
  ...
  brainReport: () => get<BrainReport>("/brain-report"),
  decisionTree: () => get<Record<string, unknown>>("/decision-tree"),
