...
>
> 🔒 **v2.0 冻结（2026-07-31 ～ 2026-09-30）**：版本锁定，期间**禁止增加字段 / 增加评分体系 / 开发 `market_linkage` 模块**。
> 仅允许：按 v2 八字段填写样本 / 每周五 Research Note 汇总 / 人工复盘。
> 解锁条件（须**全部满足**）：≥20 个有效事件 + ≥10 个失败案例 + ≥40 交易日观察，方于 Checkpoint B 升级 v2.1。
> （用户 2026-07-31 警示：继续优化模板却无真实样本 = 「研究工具优化循环」，是量化项目常见失败模式。故冻结而非实现。）
...
## 5. v2.0 Freeze（版本冻结，2026-07-31 ～ 2026-09-30）

用户 2026-07-31 明确：模板已接近「研究记录系统」成熟形态，继续优化易陷入「研究工具优化循环」
（发现不足 → 优化模板 → 发现新不足 → … → 没有真实样本）。故宣布冻结。

- **冻结版本**：Cross-Market Observation v2.0
- **观察周期**：2026-07-31 ～ 2026-09-30
- **期间允许**：✅ 按 v2 八字段填写样本 ✅ 每周五 Research Note 汇总 ✅ 人工复盘
- **期间禁止**：❌ 增加字段 ❌ 增加评分体系 ❌ 开发 `market_linkage` 模块

**升级 v2.1 的解锁条件（须全部满足）**：
- ≥ 20 个有效事件（按 §0 Event Type 标准记录的正例 + 反例）
- ≥ 10 个失败案例（Failed Transmission 反例，对 Regime Engine 最关键）
- ≥ 40 交易日观察

达到后于 **Checkpoint B（2026-09-30）** 复盘：模板是否真的缺字段，再决定 v2.1 加哪些——而非现在猜。

---

## 6. v2.1 候选 backlog（冻结期内不实现，仅记录）

以下三条优化由用户 2026-07-31 提出，经判断属「提高观察数据质量」但**当前不落地**——
冻结期内它们只作为候选 backlog，等待 v2.1 解锁。记入此而非活动模板，既防漂移，也防过早复杂化。

### C-1. Transmission Strength（传导强度等级）
| 等级 | 定义 |
|------|------|
| T0 | 无传导 |
| T1 | 指数同步 |
...
| T3 | 核心资产同步 |
| T4 | 资金链同步 |
> 用法示例：SK Hynix +8% → KOSPI +2.5% → 科创50 +3% / 半导体ETF +4% / 中芯 +5%，记 **T3（核心资产同步）**。
> 比「韩国涨→A股涨」更精确，未来能统计「T3 级传导后 A股后续收益」。

### C-2. Alternative Explanation（替代解释）
> 很多联动易错误归因。示例：韩股涨 + A股半导体涨，Hypothesis = 韩国 AI 半导体景气传导；
> Alternative = ① 美联储降息预期升温 ② 美元下跌 ③ 全球 Risk-On ④ 亚洲权益共同上涨。
> 未来验证区分「韩国领先」还是「全球 Risk-On」——避免把相关性误当因果。

### C-3. Observation Quality Score（样本质量评分）
> 不是判断市场，而是给**样本质量**打分（事件明确性 / 领先关系清晰 / 产业链一致性 / 资金确认 / 替代解释排除，各 0–10）。
> 未来 100 样本不是同价值：NVDA 财报 → SOX → 韩国半导体 → A股算力 价值高；
> 韩国指数随机涨 → A股随机涨 价值低。须区分研究资产质量。

> ⚠️ 这三条在 v2.0 冻结期内**不得写入活动模板**。若冻结期内你觉得某条必须立刻加，
> 先回到 RQ-004 与本研究问题本身——它已在 v2.1 候选里等待样本验证，而非现在实现。
> 这正是我们建立的「防研究问题漂移」纪律要保护的动作。
