...
## 14. 治理协议：Trading OS Evolution Protocol v1.0（2026-07-31 重构，原"60天冻结"升级为"60天证据积累期"）
...
### 14.3 治理升级：从「60 天冻结」到「Trading OS Evolution Protocol v1.0」

2026-07-30 立的"60 天冻结"解决了一个真问题：**每日因一个新想法就推翻昨天的设计**
（架构漂移 / Architecture Drift）。但用户 2026-07-31 复盘认定——纯冻结过严，且偏离
"AI 增强型投研系统"的本质：

- **系统太年轻**：十几天，数据结构可能还有问题（`sector_daily` 空、`market_daily` 不足、
  `flow` 结构待补）——这些是**基础建设**不是优化，冻结会拖垮后续所有研究。
- **AI 时代变化更快**：数据接口 / Agent 架构 / 研究流程迭代加速，纯冻结会错过窗口。
- **核心优势是快速学习**：方向是"会进化的 AI 投研伙伴"，不是"固定交易机器人"。

故将纪律重构为 **Trading OS Evolution Protocol v1.0（演化协议）**，并把"60 天冻结期"
重命名为 **「60 天证据积累期」**：

| 维度 | 旧：冻结期 | 新：证据积累期 |
|------|-----------|---------------|
| 系统改动 | 禁止 | 核心禁止，外围（观察/数据/研究）允许 |
| 研究 | 限制 | 鼓励 |
| 数据补充 | 限制 | 允许（P0 优先） |
| 新想法 | 保存 | 验证（进 Lifecycle） |
| 模型调整 | 禁止 | 实验沙盒隔离 |
| 目标 | 稳定 | 获得证据 |

> 最佳状态 = **核心稳定、能力迭代、实验自由**。这比单纯冻结更贴合"AI 增强型投资研究体系"。

**60 天唯一目标（一句话）**：让 Trading OS 经历真实市场、积累证据，而不是继续设计 Trading OS。
每天自动跑「数据采集 → 日报 → AIP → History」；每周人工 review（观察到什么 / 哪些判断被验证 / 哪些失败 / 当前最大未知）。

### 14.3.1 当前阶段再定义：Research Training Phase（而非 Development Phase）

系统才搭建十几天，叫它"开发阶段"会误导——仿佛主目标是把软件做得更聪明。更准确的说法：

> **Trading OS 3.0-alpha · Research Training Phase（研究训练期，2026.7–2026.12）**
> 目标不是提高预测准确率，而是建立 `Observation → Memory → Validation → Calibration` 闭环，
> 把系统训练成"会记忆、能归因"的长期数据资产。

配套时间节拍（与 Checkpoint A/B/C 对齐）：

| 阶段 | 时间窗 | 重点 | 对应节点 |
|------|--------|------|----------|
| 证据积累 | 0–60 天（≈至 2026-09-30） | 语言稳定、补数据、积累样本 | Checkpoint A(8/15) · B(9/30) |
| 第一次评审 | 60–120 天（至 2026-12-31） | 调整明显无效部分、调权重 | Checkpoint C(12/31) |
| 模型演化 | 120–365 天 | 引入验证通过的新因子 | Phase 2+ |

一句话：**不要急着让它更聪明，先让它学会记忆。** 没有历史记忆的 AI 投研系统，
本质只是个高级日报生成器。

### 14.4 三层演化模型（替代"三层冻结 / 双循环"）

```
Layer 1 核心研究协议稳定层（冻结，60天+）
   投资哲学 · 决策流程 · 数据标准 · 研究格式
        │ 闸门：Checkpoint 评审
        ▼
Layer 2 能力模块快速迭代层（开放，每天~每周）
   观察字段 · 情绪指标 · 板块轮动 · 研究模块
        │ 外循环，不进生产评分
        ▼
Layer 3 实验沙盒层（高速试错，隔离）
   Transformer / LLM Agent / ML因子 / 盘口模型
```

- **Layer 1 · 核心研究协议稳定层（冻结，硬约束，60 天甚至更长）**：以下"研究协议"（解释框架）**不变**——
  - 投资哲学：`判环境 → 选方向 → 定标的`
  - 决策流程：`Macro → Market → Sector → Stock → Risk`
  - 数据标准：股票日线格式、交易记录格式、研究笔记格式
  - 当前 L1–L8 架构骨架、风险框架
  - 目的：获得干净、可比对的历史样本；**评分模型（L4/L5 如何算分、如何驱动 IC/CIO 决策）
    是 Layer 1 的核心红线（见 14.5）**。
- **冻结的是"解释框架"，不是代码（澄清）**：Layer 1 锁死的是「`score` 代表什么 /
  决策流程怎么走 / 评分权重多少」这类**协议定义**——以下一律**允许**，不构成违反冻结：
  - 代码修 bug、补数据、优化接口 / 性能（纯工程维护）；
  - `sector_daily` / `market_daily` / `stock_flow` 等字段回填（基础设施）；
  - 研究模块、观察字段、RQ 登记（Layer 2/3 的开放动作）。
  - 一句话：**发动机可以加油、拧紧螺丝；但不能改发动机的设计图纸。**
- **Layer 2 · 能力模块快速迭代层（开放）**：可快速吸收新**观察字段**与分析能力，
  **不进生产评分**——
  - 增补资金持续性 `money_persistence`、资金偏离度、情绪指标、外围联动、Sector Rotation 分析等；
  - 如 Smart Money 的观察字段 `topic_heat / leader_status / money_persistence` 可在此层"打印观察"，
    因为它们**不改变交易结果、只增加观察能力**。
  - 本层产出归 Research Memory，仅在周报 / 观察日志中呈现，**不写入决策评分**。
- **Layer 3 · 实验沙盒层（隔离，未来）**：`experiment/` 目录放 `smart_money_v0` /
  `sector_rotation_test` / `transformer_pred` / `llm_agent_pick` 等**独立实验**；
  规则 = 可以失败、失败即删、**严禁进入生产系统**；只有通过样本验证的因子，
  才经 Checkpoint 评审升入 Layer 1。

### 14.5 评分模型冻结（Layer 1 的核心红线）

Layer 2 允许加观察字段 / 补数据 / 写研究模块，但**以下一律冻结至 Checkpoint 评审**：

- ❌ 修改 L4（资金共识）/ L5（龙头选择）的**评分公式与权重**
- ❌ 修改 `score → IC → CIO` 的**决策驱动逻辑**
- ❌ 把任何未验证观察字段直接接入生产评分

理由：不知道它是否有效前接入，三个月后赚钱也无法归因"到底谁贡献"。Smart Money / 跨市场因子
必须先以**观察字段**形式零风险累积，经分组回测验证后才进评分。

### 14.6 数据补洞与交易记录（P0，允许且优先）

证据积累期**允许并优先**做基础建设（非架构漂移）：

1. **数据质量（P0-1）**：回填 `sector_daily.net_amount / sector_score`（板块资金流，
   截至 2026-07-17 仍空）、补齐 `market_daily` 情绪/涨跌家数（最近有效日 2026-07-14）、
   `stock_flow_daily` 真实资金流（7-20 起）、全球市场 `global_history`。
   垃圾输入 → 垃圾研究，这是"选方向"第二步断点的头号修复项。
2. **交易记录体系（P0-2，新建但属证据基建）**：结构化记录每笔的「买入原因 / 卖出原因 /
   结果 / 错误类型」——这是未来训练 AI Research Agent 的**私人知识资产**，
   不引入交易信号，仅作因果记录。
3. **Research Memory（P0-3）**：继续累积 Smart Money / Cross-Market / 宏观判断 / 失败案例。

> 注：P0 数据补洞是"修复既有字段 / 补齐既有表"，**不是新增信号型表/列**；
> 新增信号列按 14.5 走 Layer 2 → 评审。

### 14.7 Research Asset Lifecycle（研究资产生命周期治理，2026-07-31 用户治理升级）

> **问题意识**：Layer 2 研究层"每天开放"若不治理，会逐渐变成"知识垃圾场"——
> 大量 Idea / Note / 假设无限挂起、无人收口。必须给每个研究资产一个**显式状态**，
> 且只允许在约定阶段内流动。

状态机（每个研究资产必须处于其中之一）：

```
Idea → Research Note → Hypothesis → Observation → Experiment → Validated → Production → Rejected
```

| 状态 | 含义 | 允许的产出 / 流动 |
|------|------|------------------|
| Idea | 灵感 / 碎片 | 仅记一笔，不展开 |
| Research Note | 已结构化拆解（如 RN-2026-08） | 冻结期可产 |
| Hypothesis | 已提炼为可验证假设（H1/H2/H3、RQ-001~004） | 冻结期可产（钉死，不漂移） |
| Observation | 正在累积样本（Cross-Market v2、Smart Money 失败库） | 冻结期可产 |
| Experiment | 隔离实验（experiment/ 目录） | 仅未来，禁入生产 |
| Validated | 样本验证通过，可升生产 | Checkpoint 评审后 |
| Production | 已进入 L1–L8 | — |
| Rejected | 证伪 / 过时 / 无用 | 归档，不再投入 |

**当前研究资产状态登记（防无限挂起）：**

| 资产 | 当前状态 | 冻结期动作 |
|------|----------|-----------|
| Cross-Market Observation v2.0 | Observation（冻结至 2026-09-30） | 仅填样本 + 周报 + 复盘 |
| Smart Money & Momentum Module | Hypothesis（H1/H2/H3 钉死，未观察） | 仅文档，禁进代码 |
| Commodity OS（commodity_engine） | Production（Phase 1.x 已上线，验证中） | 稳定累积，不重构 |
| RQ-001~004（含 RQ-004 跨市场） | Hypothesis（钉死，待 Checkpoint 验证） | 每周同步状态 |
| 失败案例种子库（Smart Money §4 + 跨市场反例） | Observation（种子已记，未系统化） | 随样本补充 |

> 任何新研究资产**必须**先登记状态再展开；冻结期内只允许 `Idea → Research Note →
> Hypothesis → Observation` 四态流动，`Experiment / Validated / Production / Rejected`
> 一律留到 Checkpoint 评审后。

### 14.8 证据积累期解锁条件（样本驱动，双条件）

60 天不是为"等时间过去"，而是为"获取样本"。Checkpoint C（2026-12-31）解锁须**双条件**同时满足：

- **时间条件**：≥ 40 个交易日运行记录（每日落库 100%）。
- **证据条件**（四项全满足）：

  1. ≥ 3 种 Regime 类型被真实经历（Risk-On / 震荡 / Risk-Off 至少都覆盖，而非仅数"切换次数"）
  2. ≥ 30 个候选标的跟踪（来自 Research Memory 观察池）
  3. ≥ 10 个失败案例沉淀（Smart Money 失败约束库、Cross-Market 反例等）
  4. ≥ 1 次系统复盘报告（Beta/Alpha 归因，呼应 H2；明确"赚钱来自环境还是选股"）

> ⚠️ **解锁 ≠ 自动开发**：满足后进入 **Trading OS 2.0 Review（评审）**，由评审决定哪些
> Research Memory 资产升入 Experiment / Production，而非系统自动改架构、加因子。

### 14.9 优先级排序（P0 / P1 / P2）

| 层级 | 时间窗 | 内容 | 说明 |
|------|--------|------|------|
| **P0** | 现在立即 | 数据质量（sector_daily / market sentiment / global / stock flow）· 交易记录体系 · Research Memory | 垃圾输入→垃圾研究；证据基建优先 |
| **P1** | 1–2 个月 | 验证 Market Regime · Sector Rotation · Stock Ranking | 用积累样本做分组回测，判断现有能力是否有效 |
| **P2** | 3 个月以后 | 自动评分调整 · Agent · ML 模型 | 仅在 P1 验证通过后才考虑，避免架构漂移 |

> ⚠️ 积累期最大风险 = **研究问题漂移**（research question drift）：8月研究 score 有效性、
> 9月突然想加黄金模型、10月又调情绪指标——最终没有一个问题被完整验证。
> RQ 登记册 + 三阶段 Checkpoint + Lifecycle 状态机，正是为了把问题「钉死」在样本积累上。
